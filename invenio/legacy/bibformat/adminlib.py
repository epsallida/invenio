            ...
            if line.strip().endswith(":") or (line.strip().lower().startswith("tag") and line.find('---') == -1):
                ...
                if len(clean_line.split()) > 2: #We should only have 'tag' keyword and tag
                    ...
                    if len(check_tag(current_tag)) > 0:
                        # Invalid tag
                        try:
                            raise InvenioBibFormatError(_('Invalid tag "%(x_line)s" at line %(x_col)s.',
                                    x_line=current_tag, x_col=i))
                        except InvenioBibFormatError, exc:
                            register_exception()
                            errors.append(exc.message)
    ...
    if can_read_format_template(filename):#Can template be read?
        if filename.endswith('.'+CFG_BIBFORMAT_FORMAT_TEMPLATE_EXTENSION):
            ...
            if match is None:#Is tag <name> defined in template?
                try:
                    raise InvenioBibFormatError(_('Could not find a name specified in tag "<name>" inside format template %(x_file)s.', x_file=filename))
                except InvenioBibFormatError, exc:
                    register_exception()
                    errors.append(exc.message)
            ...
            if match is None:#Is tag <description> defined in template?
                try:
                    raise InvenioBibFormatError(_('Could not find a description specified in tag "<description>" inside format template %(x_file)s.', x_file=filename))
                except InvenioBibFormatError, exc:
                    register_exception()
                    errors.append(exc.message)
            ...
            for element_match in elements_call:
                ...
                if filename is None and not bibformat_dblayer.tag_exists_for_name(element_name): #Is element defined?
                    try:
                        raise InvenioBibFormatError(_('Format template %(x_file)s calls undefined element "%(x_elem)s".', x_file=filename, x_elem=element_name))
                    except InvenioBibFormatError, exc:
                        register_exception()
                        errors.append(exc.message)
                else:
                    ...
                    if format_element is None:#Can element be loaded?
                        if not can_read_format_element(element_name):
                            try:
                                raise InvenioBibFormatError(_('Format template %(x_file)s calls unreadable element "%(x_elem)s". Check element file permissions.', x_file=filename, x_elem=element_name))
                            except InvenioBibFormatError, exc:
                                register_exception()
                                errors.append(exc.message)
                        else:
                            try:
                                raise InvenioBibFormatError(_('Cannot load element "%(x_file)s" in template %(x_elem)s. Check element code.', x_file=filename, x_elem=element_name))
                            except InvenioBibFormatError, exc:
                                register_exception()
                                errors.append(exc.message)
                    else:
